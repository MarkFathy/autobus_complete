part of '../screens/about_game_screen.dart';

class AboutGameScreenBody extends StatelessWidget {
  const AboutGameScreenBody({super.key});

  @override
  Widget build(BuildContext context) => BlocProvider(
    create: (_) {
      final cubit = sl<AppInfoCubit>();
      unawaited(cubit.loadAboutGame());
      return cubit;
    },
    child: BlocBuilder<AppInfoCubit, AppInfoState>(
      builder: (context, state) {
        ...
        final title = state is AppInfoLoaded ? state.info.getLocalizedTitle(context) : '';
        final content = state is AppInfoLoaded ? state.info.getLocalizedContent(context) : '';
